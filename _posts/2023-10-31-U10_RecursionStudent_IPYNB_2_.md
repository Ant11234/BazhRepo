...
- Something calling itself to repeat a specified number of times.
...
```Java
public class LowestPrimeFinder {
    public static int findLowestPrime(int n) {
        if (n <= 1) {
            return 2;  // The lowest prime number is 2
        }
        return findLowestPrimeHelper(n, 2);
    }

    public static int findLowestPrimeHelper(int n, int divisor) {
        if (n % divisor == 0) {
            return divisor;  // Found the lowest prime divisor
        }
        if (divisor * divisor > n) {
            return n;  // n is a prime number
        }
        return findLowestPrimeHelper(n, divisor + 1);
    }

    public static void main(String[] args) {
        int input = 20;
        int lowestPrime = findLowestPrime(input);
        System.out.println("The lowest prime number in " + input + " is: " + lowestPrime);
    }
}

LowestPrimeFinder.main(null);
```

    The lowest prime number in 20 is: 2
...
- Capturing progression is used in artificial intelligence to track performance of algorithms.
...
- So that the iterate version only goes down to one like the recursive function
...
public class RangeExtractor {
    public static void main(String[] args) {
        String input = "This is a sample text for range extraction.";

        // Define two ranges (start and end indices) for extraction
        int range1Start = 5;
        int range1End = 16;
        int range2Start = 20;
        int range2End = 28;

        String extractedRange1 = extractRange(input, range1Start, range1End);
        String extractedRange2 = extractRange(input, range2Start, range2End);

        System.out.println("Range 1: " + extractedRange1);
        System.out.println("Range 2: " + extractedRange2);
    }

    public static String extractRange(String input, int start, int end) {
...
public class String2DArrayTraversal {
    public static void main(String[] args) {
        ...
        // Prompt the user for a letter to search for
        Scanner scanner = new Scanner(System.in);
        System.out.print("Enter a letter to search for: ");
        char targetLetter = scanner.nextLine().charAt(0);

        // Recursively traverse and search the 2D array
        System.out.println("Strings containing the letter '" + targetLetter + "':");
        traverseString2DArray(array2D, targetLetter, 0, 0);
    }

    public static void traverseString2DArray(String[][] array, char targetLetter, int row, int col) {
        if (row >= array.length) {
            return; // Base case to stop the recursion
        }
        
        if (col >= array[row].length) {
            traverseString2DArray(array, targetLetter, row + 1, 0); // Move to the next row
            return;
        }
        
        String currentString = array[row][col];
        if (containsLetter(currentString, targetLetter)) {
            System.out.println(currentString);
        }
        
        traverseString2DArray(array, targetLetter, row, col + 1); // Move to the next column
    }
...
![img](https://github.com/Ant11234/student/assets/40652645/5cdee616-6677-4317-8d82-8c1531cc48da)


# Recursion
- You use recursion in order to repeatedly sort each terms individually. Merge sort is done with recursion because you have to repetedly compare individual terms.

...
Sets of images are used to train neural networks because the order in unimportant and when training an AI duplicates are pointless(dont know if this is what you meant by real world example, if not ignore this are read the next example)

A queue can be used whenever you're waiting in line in order to keep the order the same and make sure people that were first in are first out, making it so that people who were there the longest get served first.
...
- findByEmail() - Finds information about a person using their email.
- findByEmailAndPassword() - Finds information about a person using their email and password.
- findAllByOrderByNameAsc() - Finds in ascending alphabetical order.
findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase() - Finds while ignoring whether or not the characters are upper or lower case.
- findByLikeTermNative() - Find by using similar terms.
...
PersonRole is a subsection/attribute of Person object, and you can have multiple PersonRole in Person object. This is relevant to collections because with it you can store multiple people in a single role.
...
"tutorial_id" is on the rightmost column on the table and it means that because of the nullable = false, the leftmost "id" cannot be empty.
...
<img width="1000" alt="Screenshot 2024-01-09 at 12 46 28 AM" src="https://github.com/Ant11234/student/assets/40652645/d086e5da-7b01-4691-8970-389d5a054b88">
